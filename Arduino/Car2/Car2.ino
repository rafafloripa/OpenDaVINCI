...
void setup() {
  ...
  IRRB.begin();
  IRB.begin();
  USFC.connect(AddressFrontCenter, GAIN_REGISTER, LOCATION_REGISTER);
  USFR.connect(AddressFrontRight, GAIN_REGISTER, LOCATION_REGISTER);
  servos.begin();
...
void sendData() {
  if (millis() >= (startTimeData+100)) {                                    // Send sensor data ten times per second (every 100ms)
    float usfc = USFC.getRange('c');
    float usfr = USFR.getRange('c');
    float irrf = IRRF.getDistance();
    float irrb = IRRB.getDistance();
    float irb = IRB.getDistance();
    String gyro = "-1";//getLastSerialGyro();
